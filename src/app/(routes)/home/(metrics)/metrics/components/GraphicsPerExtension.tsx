...
const NameMetric = "Files Per Extension"
...
export default function GraphicsFilesPerExtension({ data }: Props) {
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	const chartData: any[] = Object.entries(data?.files_per_extension).map(([ext, count], index) => ({
		browser: ext,
		data: count,
		...
		<Card className="flex flex-col bg-zinc-900 grow">
			...
			<CardFooter className="flex-col gap-2 text-sm">
				<div className="flex items-center gap-2 font-medium leading-none">
					The {chartData[0].browser.substring(1).toUpperCase()} has more {NameMetric}.
				</div>
